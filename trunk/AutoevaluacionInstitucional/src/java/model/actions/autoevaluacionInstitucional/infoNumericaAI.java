...
public class infoNumericaAI implements Action {
    ...
    public String procesar(HttpServletRequest request)
        ...
        Result rs = null;
        rs = conSql.CargarSql2("Select indicador.id, indicador.codigo, factor.id from indicador inner join instrumentohasindicador on indicador.id = instrumentohasindicador.indicador_id inner join caracteristica on caracteristica.id = indicador.caracteristica_id inner join factor on factor.id=caracteristica.factor_id where instrumentohasindicador.instrumento_id = 3 order by indicador.id", bd);
