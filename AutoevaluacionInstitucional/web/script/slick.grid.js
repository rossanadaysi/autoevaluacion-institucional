 ...
 *     autoEdit                 - (default true) la celda automaticamente entra en modo edit cuando se selecciona.
 ...
 *     enableColumnReorder      - (default true) permite cambiar el orden de las columnas.
...
(function($) {
    ...
    function SlickGrid(container,data,columns,options) {
        ...
        function setupColumnResize() {
            ...
            columnElements.each(function(i,e) {
                if ((firstResizable !== undefined && i < firstResizable) || (options.forceFitColumns && i >= lastResizable)) {
                    return;
                }
                $col = $(e);
    ...
    function styleColumnWidth(index,width,styleCells) {
        ...
        if (styleCells) {
            if(index==0 || index+1 ==columns.length ){
                findCssRuleForCell(index).style.width = (width - cellWidthDiff - 10) + "px";
            }else{
    ...
    function appendRowHtml(stringArray,row) {
        ...
        /*modificado x mi*/
        stringArray.push("<div class='ui-widget-content " + css + "' row='" + row + "' style='top:" + (options.rowHeight*row-offset +10 ) + "px'>");
        var ancho="";
        var n=20;
        for (var i=0, cols=columns.length; i<cols; i++) {
            ...
            /*
                if(i==0 || i+1==cols){
                      ...
                      ancho="width: "+((m.currentWidth||m.width)-n)+"px;";
                      stringArray.push("<div class='" + cellCss + " ggg ' style='"+ancho+"'>");
    ...
    function gotoDir(dy, dx, rollover) {
        ...
        if (nextCell && !nextCell.length) {
            ...
            .filter(function(index) {
                return (dx>0) ? index > currentCell + dx : index < currentCell + dx
            })
        ...
        if (currentEditor) {
            if (currentEditor.isValueChanged()) {
                ...
                if (validationResults.valid) {
                    ...
                    $(currentCellNode).stop(true,true).effect("highlight", {
                        color:"red"
                    }, 300);
